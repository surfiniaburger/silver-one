...
[ Seed Candidate (Predicate + Code) ]
                 ...
                 ▼
 ┌───────────────────────────────┐
 │ Stage A: Heuristic Rules      │ ──► Confident? ──► [ Return Decision (<0.1ms) ]
 └───────────────┬───────────────┘
                 │ Uncertain
                 ▼
 ┌───────────────────────────────┐
 ...
 └───────────────┬───────────────┘
                 ...
                 ▼
 ┌───────────────────────────────┐
 │ Stage C: SetFit (Transformer) │ ──► Final Semantic Decision (~10ms)
 └───────────────────────────────┘
```
...
- **Target Files**:
  - `[NEW]` [scenarios/debate/pre_filter.py](file:///Users/surfiniaburger/Desktop/modular-metacog-swarm-v3/agent_training/silver-one/scenarios/debate/pre_filter.py)
  - `[NEW]` [scenarios/debate/test_pre_filter.py](file:///Users/surfiniaburger/Desktop/modular-metacog-swarm-v3/agent_training/silver-one/scenarios/debate/test_pre_filter.py)
- **Key Features**:
  ...
  - Graceful passthrough (`default_pass`) when model weights are not yet generated.
...
- **Target Files**:
  - `[NEW]` [scripts/train_pre_filter.py](file:///Users/surfiniaburger/Desktop/modular-metacog-swarm-v3/agent_training/silver-one/scripts/train_pre_filter.py)
  - `[NEW]` [tests/test_train_pre_filter.py](file:///Users/surfiniaburger/Desktop/modular-metacog-swarm-v3/agent_training/silver-one/tests/test_train_pre_filter.py)
- **Key Features**:
...
- **Target Files**:
  - `[MODIFY]` [scenarios/debate/run_batch.py](file:///Users/surfiniaburger/Desktop/modular-metacog-swarm-v3/agent_training/silver-one/scenarios/debate/run_batch.py)
  - `[MODIFY]` [scenarios/debate/adk_debate_judge.py](file:///Users/surfiniaburger/Desktop/modular-metacog-swarm-v3/agent_training/silver-one/scenarios/debate/adk_debate_judge.py)
  - `[MODIFY]` [scenarios/debate/test_adk_debate_judge_refactor.py](file:///Users/surfiniaburger/Desktop/modular-metacog-swarm-v3/agent_training/silver-one/scenarios/debate/test_adk_debate_judge_refactor.py)
- **Key Features**:
  - `run_batch.py`: Add `--pre-filter / --no-pre-filter` CLI argument. Intercept seeds in `_process_seed()` before sending HTTP JSON-RPC payloads to the green agent server. Record `"skipped_pre_filter"` status in manifest when rejected.
...
- **Target Files**:
  - `[NEW]` [reports/pre_filter_benchmark.md](file:///Users/surfiniaburger/Desktop/modular-metacog-swarm-v3/agent_training/silver-one/reports/pre_filter_benchmark.md)
  - `[MODIFY]` [README.md](file:///Users/surfiniaburger/Desktop/modular-metacog-swarm-v3/agent_training/silver-one/README.md)
  - `[MODIFY]` [docs/EVALUATOR_LATENCY_AND_CACHE_BENCHMARKS.md](file:///Users/surfiniaburger/Desktop/modular-metacog-swarm-v3/agent_training/silver-one/docs/EVALUATOR_LATENCY_AND_CACHE_BENCHMARKS.md)
- **Key Features**:
...
- Verify `<10ms` latency threshold for 100 sample pre-filter predictions.

### Manual Verification
- Execute `run_batch.py` on `scenarios/debate/cve_seeds_test.jsonl` with `--pre-filter`.
- Verify in `artifacts/attempts/` and batch manifests that bad seeds are recorded as `"skipped_pre_filter"` without consuming LLM API tokens.
